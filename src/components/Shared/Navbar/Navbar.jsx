...
import { LogIn, LogOut, Menu, User, Home, Newspaper, ArrowRightLeft, BarChart3 } from "lucide-react"
...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { href: "/news", label: "All News", icon: <Newspaper className="w-4 h-4 mr-2" /> },
    { href: "/transfers", label: "Transfer News", icon: <ArrowRightLeft className="w-4 h-4 mr-2" /> },
    { href: "/rankings", label: "Power Rankings", icon: <BarChart3 className="w-4 h-4 mr-2" /> },
  ]
                ...
                <div className="flex flex-col space-y-1">
                  {navLinks.map((link) => (
                    <Link key={link.href} href={link.href}>
                      <Button variant={path === link.href ? "secondary" : "ghost"} className="w-full justify-start">
                        {link.icon}
                        {link.label}
                      </Button>
                    </Link>
